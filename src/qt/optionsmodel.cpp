...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeECKOAmount"))
        settings.setValue("nAnonymizeECKOAmount", 1000);
    nAnonymizeECKOAmount = settings.value("nAnonymizeECKOAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeECKOAmount"))
        SoftSetArg("-anonymizeECKOamount", settings.value("nAnonymizeECKOAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeECKOAmount:
            return QVariant(nAnonymizeECKOAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeECKOAmount:
            nAnonymizeECKOAmount = value.toInt();
            settings.setValue("nAnonymizeECKOAmount", nAnonymizeECKOAmount);
            emit AnonymizeECKOAmountChanged(nAnonymizeECKOAmount);
            break;
        default:
            break;
        }
