...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ECKO
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ECKO
    // Mac: ~/Library/Application Support/ECKO
    // Unix: ~/.ECKO
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ECKO";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "ECKO.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
    {
        boost::filesystem::path ConfPath;
               ...
               fprintf(ConfFile, "port=15391\n");
               fprintf(ConfFile, "rpcport=15392\n");
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "ECKOd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong ECKO will not work properly.");
