...
const string strMessageMagic = "ECKO Signed Message:\n";
...
bool AcceptToMemoryPool(CTxMemPool& pool, CTransaction &tx, bool fLimitFree,
                        bool* pfMissingInputs, bool fRejecteckoFee, bool ignoreFees)
{
    AssertLockHeld(cs_main);
    if (pfMissingInputs)
...

    {
        ...
        if (fRejecteckoFee && nFees > MIN_RELAY_TX_FEE * 10000)
            return error("AcceptableInputs: : ecko fees %s, %d > %d",
                         hash.ToString(),
...
bool AcceptableInputs(CTxMemPool& pool, const CTransaction &txo, bool fLimitFree,
                         bool* pfMissingInputs, bool fRejecteckoFee, bool isDSTX)
{
    AssertLockHeld(cs_main);
    if (pfMissingInputs)
        *pfMissingInputs = false;
...

    {
        ...
        if (fRejecteckoFee && nFees > txMinFee * 10000)
            return error("AcceptableInputs: : ecko fees %s, %d > %d",
                         hash.ToString(),
                         nFees, MIN_RELAY_TX_FEE * 10000);
...
bool CMerkleTx::AcceptToMemoryPool(bool fLimitFree, bool fRejecteckoFee, bool ignoreFees)
{
    return ::AcceptToMemoryPool(mempool, *this, fLimitFree, NULL, fRejecteckoFee, ignoreFees);
}
