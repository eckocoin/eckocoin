...
namespace {
    class CECKOAddressVisitor : public boost::static_visitor<bool> {
    private:
        CECKOAddress *addr;
    public:
        CECKOAddressVisitor(CECKOAddress *addrIn) : addr(addrIn) { }
...
bool CECKOAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CECKOAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CECKOAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CECKOAddressVisitor(this), dest);
}

bool CECKOAddress::IsValid() const {
...
CTxDestination CECKOAddress::Get() const {
    if (!IsValid())
...
bool CECKOAddress::GetKeyID(CKeyID &keyID) const {
...
bool CECKOAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CECKOSecret::SetKey(const CKey& vchSecret) {
...
CKey CECKOSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CECKOSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CECKOSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CECKOSecret::SetString(const std::string& strSecret) {
